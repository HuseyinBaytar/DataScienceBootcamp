...
#%timeit -r1 -n3
df["Hizmet"] = [f"{O}_{o}" for O,o in zip(df["ServiceId"],df["CategoryId"])]
...
#%timeit -r1 -n3 df['Hizmet'] = df.apply(lambda x: str(x['ServiceId']) + '_' + str(x['CategoryId']), axis=1) #Hüseyin
...
pivoteddf = df.groupby(["SepetID","Hizmet"])["Hizmet"].count().unstack().fillna(0).applymap(lambda x: True if x>0 else False)
